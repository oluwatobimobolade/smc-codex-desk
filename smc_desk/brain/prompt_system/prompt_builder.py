...
def build_layered_ai_smc_prompt(evidence_pack: Mapping[str, Any]) -> str:
    ...
    prompt = {
        ...
        "non_negotiables": [
            ...
            "Read formal_causal_episode_graph before formal_structure_graph. The episode graph is downgrade-only and records whether each V1 controlling break survives stricter displacement and acceptance replay. If its invariants are not PASS, return REVIEW_REQUIRED and do not preserve a contradicted POI or structure mark.",
            "Annotate one causal story, not unrelated detections: protected origin, relevant liquidity interaction, accepted displacement/break, active POI role, confirmation condition, and unswept objective.",
            ...
            "Read causal_poi_authority after the formal graph and before naming any active POI. A geometric OB/FVG candidate is not a primary POI. The selected controlling origin must remain in active_poi.evidence_object_ids even when a lower-timeframe refinement is used.",
            "Never select an OB merely because it is nearest, deepest, newest, or the last opposing candle. Causal lineage and protected-origin ownership come first; range location, lifecycle, departure quality, and refinement follow; depth is only a tie-break.",
            "An FVG supports displacement or refines an eligible causal OB. It may be primary only when causal_poi_authority explicitly classifies it standalone_fvg_primary.",
        ],
        "mandatory_self_review": [
            "First pass: map HTF bias, active range, liquidity, sweep/displacement, POI, entry readiness.",
            "Episode pass: replay the formal_causal_episode_graph sequence and remove any thesis or annotation whose controlling break was challenged by StructureEngineV3Shadow.",
            ...
            "POI challenger pass: compare the selected origin against every eligible alternative and state why the loser lost. If the causal origin is unresolved, remove the POI rather than guessing.",
