...
canonical PerceptionEngineV2 path. The cutover commit (WP-SMC-10/3) flipped
all three defaults to ON. Read at call time (not import time) so tests can
...
def canonical_displacement_scoring_enabled() -> bool:
    ...
    of the legacy hardcoded 0.0. Default ON as of WP-SMC-10/3 cutover."""
    return os.environ.get(_ENV_DISPLACEMENT, "1") == "1"
...
def causal_protected_point_enabled() -> bool:
    ...
    Default ON as of WP-SMC-10/3 cutover."""
    return os.environ.get(_ENV_PROTECTED_POINT, "1") == "1"
...
def causal_ob_origin_gate_enabled() -> bool:
    ...
    Default ON as of WP-SMC-10/3 cutover."""
    return os.environ.get(_ENV_ORIGIN_GATE, "1") == "1"
